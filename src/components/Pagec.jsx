import React from 'react';
import build from "../assets/g.png";
import laptop2 from "../assets/laptop22.png";
const Pagec = () => {
    return (
        <>
        <section >
        <div className="jumbotron mb-0 mt-0 pt-0">
          <div className="row mt-0">
            <div className="col-md-12 d-flex justify-content-between ml-3 mt-4">
              <h2
                className="text-primary "
                style={{ display: "inline-block", fontWeight: "bolder" }}
              >
                Properties for Sale
              </h2>
              <span
                className=" text-warning mr-2 h4"
                style={{ fontSize: "x-large", fontWeight: "normal" }}
              >
                Display all <i class="fa-solid fa-arrow-right"></i>
              </span>
            </div>
          </div>

          <div className="row mt-4">
            <div className="col-md-2 ">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                Laptops for Sell
              </button>
            </div>
           
            <div className="col-md-2">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                Dell used Laptops
              </button>
            </div>
            <div className="col-md-2">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                Gaming Laptops
              </button>
            </div>
            <div className="col-md-2">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                New Laptops
              </button>
            </div>
            <div className="col-md-2">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                MacBook Pro
              </button>
            </div>
          </div>
          <div className="row mt-5">
          <div className="col-md-3   ">
              <div
                class="card text-center"
                style={{ width: "16rem", borderRadius: "30px" }}
              >
                <img
                  class="card-img-top card-header pl-0 pr-0 pt-0 pb-0"
                  src={laptop2}
                  alt="Card image cap"
                 
                />
                <div class="card-body ">
                  
                <button
                type="button"
                style={{ width: "100%"}}
                class="btn btn-outline-primary pt-2 pb-2 text-secondary mt-1"
              >
                150,000,0 OMR,Commercial
              </button>
           
                </div>
              </div>
            </div>
            <div className="col-md-3   ">
              <div
                class="card text-center"
                style={{ width: "16rem", borderRadius: "30px" }}
              >
                <img
                  class="card-img-top card-header pl-0 pr-0 pt-0 pb-0"
                  src={laptop2}
                  alt="Card image cap"
                 
                />
                <div class="card-body ">
                  
                <button
                type="button"
                style={{ width: "100%"}}
                class="btn btn-outline-primary pt-2 pb-2 text-secondary mt-1"
              >
                150,000,0 OMR,Commercial
              </button>
           
                </div>
              </div>
            </div>
            <div className="col-md-3   ">
              <div
                class="card text-center"
                style={{ width: "16rem", borderRadius: "30px" }}
              >
                <img
                  class="card-img-top card-header pl-0 pr-0 pt-0 pb-0"
                  src={laptop2}
                  alt="Card image cap"
                 
                />
                <div class="card-body ">
                  
                <button
                type="button"
                style={{ width: "100%"}}
                class="btn btn-outline-primary pt-2 pb-2 text-secondary mt-1"
              >
                150,000,0 OMR,Commercial
              </button>
           
                </div>
              </div>
            </div>
            <div className="col-md-3   ">
              <div
                class="card text-center border"
                style={{ width: "16rem", borderRadius: "30px" }}
              >
                <img
                  class="card-img-top card-header pl-0 pr-0 pt-0 pb-0"
                  src={laptop2}
                  alt="Card image cap"
                 
                />
                <div class="card-body ">
                  
                <button
                type="button"
                style={{ width: "100%"}}
                class="btn btn-outline-primary pt-2 pb-2 text-secondary mt-1"
              >
                150,000,0 OMR,Commercial
              </button>
           
                </div>
              </div>
            </div>
          </div>
        
      
          <div className="row mt-5">
            <div className="col-md-12 d-flex justify-content-between ml-3">
              <h2
                className="text-primary"
                style={{ display: "inline-block", fontWeight: "bolder" }}
              >
                Properties for Sale
              </h2>
              <span
                className=" text-warning mr-2 h4"
                style={{ fontSize: "x-large", fontWeight: "normal" }}
              >
                Display all <i class="fa-solid fa-arrow-right"></i>
              </span>
            </div>
          </div>

          <div className="row mt-4">
            <div className="col-md-2 ">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                Apartments for Sale
              </button>
            </div>
           
            <div className="col-md-2">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                Villa place for Sale
              </button>
            </div>
            <div className="col-md-2">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                Commercial for Sale
              </button>
            </div>
            <div className="col-md-2">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1 inline"
              >
                Foreign real estate
              </button>
            </div>
            <div className="col-md-2">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                Town House For Sale
              </button>
            </div>
            <div className="col-md-2">
              <button
                type="button"
                style={{ width: "100%" }}
                class="btn btn-outline-dark pt-2 pb-2 mt-1"
              >
                Whole Buidling sale
              </button>
            </div>
          </div>
          <div className="row mt-5">
          <div className="col-md-3   ">
              <div
                class="card text-center"
                style={{ width: "16rem", borderRadius: "30px" }}
              >
                <img
                  class="card-img-top card-header pl-0 pr-0 pt-0 pb-0"
                  src={build}
                  alt="Card image cap"
                 
                />
                <div class="card-body ">
                  
                <button
                type="button"
                style={{ width: "100%"}}
                class="btn btn-outline-primary pt-2 pb-2 text-secondary mt-1"
              >
                150,000,0 OMR,Commercial
              </button>
           
                </div>
              </div>
            </div>
            <div className="col-md-3   ">
              <div
                class="card text-center"
                style={{ width: "16rem", borderRadius: "30px" }}
              >
                <img
                  class="card-img-top card-header pl-0 pr-0 pt-0 pb-0"
                  src={build}
                  alt="Card image cap"
                 
                />
                <div class="card-body ">
                  
                <button
                type="button"
                style={{ width: "100%"}}
                class="btn btn-outline-primary pt-2 pb-2 text-secondary mt-1"
              >
                150,000,0 OMR,Commercial
              </button>
           
                </div>
              </div>
            </div>
            <div className="col-md-3   ">
              <div
                class="card text-center"
                style={{ width: "16rem", borderRadius: "30px" }}
              >
                <img
                  class="card-img-top card-header pl-0 pr-0 pt-0 pb-0"
                  src={build}
                  alt="Card image cap"
                 
                />
                <div class="card-body ">
                  
                <button
                type="button"
                style={{ width: "100%"}}
                class="btn btn-outline-primary pt-2 pb-2 text-secondary mt-1"
              >
                150,000,0 OMR,Commercial
              </button>
           
                </div>
              </div>
            </div>
            <div className="col-md-3   ">
              <div
                class="card text-center"
                style={{ width: "16rem", borderRadius: "30px" }}
              >
                <img
                  class="card-img-top card-header pl-0 pr-0 pt-0 pb-0"
                  src={build}
                  alt="Card image cap"
                 
                />
                <div class="card-body ">
                  
                <button
                type="button"
                style={{ width: "100%"}}
                class="btn btn-outline-primary pt-2 pb-2 text-secondary mt-1"
              >
                150,000,0 OMR,Commercial
              </button>
           
                </div>
              </div>
            </div>
          </div>
          </div>
       
      </section>
            <section>
              <div className="container-fluid bg-primary mb-0">
                <div className="jumbotron bg-primary text-light  mb-2">
                  <div className="row">
                    <div className="col-md-12">
                    <h1 className='text-center text-white' style={{fontFamily:"verdana"}}>
                 Create Your Own Online Shop Today!
                 </h1>
                 <div className='mt-5 ml-3'>
                  <ul className='mt-2'>
                <li className='mt-2 h6' style={{wordSpacing:"6px",fontFamily:"cursive"}}>Unleash Your Entrepreneurial Spirit</li>
                    <li className='mt-3 h6'style={{wordSpacing:"6px",fontFamily:"cursive"}}> Launch Your Business in Minutes</li>
                    <li className='mt-3 h6'style={{wordSpacing:"6px",fontFamily:"cursive"}}>Maximize Your Profits, Minimize Your Efforts</li>
                    <li className='mt-3 h6'style={{wordSpacing:"6px",fontFamily:"cursive"}}>Seamiless Shopping EXperieance for Your Customers</li>
                  </ul>
                 </div>
                 <div className='text-center mt-5'>
             
<button type="button" class="btn btn-outline-light text-light btn-lg mt-3" style={{width:"40%"}}>Create Now</button>

                 </div>

                    </div>
                  </div>
                 
                
                  
                  
                </div>
              </div>
            </section>
          
        </>
    );
}

export default Pagec;
